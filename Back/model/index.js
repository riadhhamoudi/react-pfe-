...
const getFacturesByUserId = async (user_id) => {
  const [rows] = await db.query('SELECT * FROM facture WHERE user_id = ?', [user_id]);
  return rows;
};
...
const postArchive = async (fournisseur, dossier, date_fact, periode_conso, num_fact, device, montant, objet, num_po, statut, pathpdf, date_receprion) => {
  if (!date_fact) {
    throw new Error('date_fact ne peut pas être vide');
  }
...
const postFactureP = async (fournisseur, dossier, date_fact, periode_conso, num_fact, device, montant, objet, num_po, statut, pathpdf, date_receprion) => {
  if (!date_fact) {
    throw new Error('date_fact cannot be empty');
  }
  await db.query('INSERT INTO trait_fact (fournisseur, dossier, date_fact, periode_conso, num_fact, device, montant, objet, num_po, statut, pathpdf, date_receprion) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [fournisseur, dossier, date_fact, periode_conso, num_fact, device, montant, objet, num_po, statut, pathpdf, date_receprion]);
};

const getAllFacturesP = async () => {  
  const [rows] = await db.query('SELECT * FROM trait_fact');  
  return rows;
 }; 



 const acceptFacture = async (id) => {
  const connection = await db.getConnection(); // Get a connection from the pool
  try {
    // Start a transaction
    await connection.beginTransaction();
    ...
    // Get the facture by id
    const [factureRows] = await connection.query('SELECT * FROM trait_fact WHERE id = ?', [id]);
    if (factureRows.length === 0) {
    ...
    // Insert the facture into the archive table
    await connection.query('INSERT INTO facture (fournisseur, dossier, date_fact, periode_conso, num_fact, device, montant, objet, num_po, statut, pathpdf, date_receprion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    [facture.fournisseur, facture.dossier, facture.date_fact, facture.periode_conso, facture.num_fact, facture.device, facture.montant, facture.objet, facture.num_po, facture.statut, facture.pathpdf, facture.date_receprion]);

    // Delete the facture from the facture table
    await connection.query('DELETE FROM trait_fact WHERE id = ?', [id]);
...
const deletefactureP = async (id) => {
  await db.query('DELETE FROM trait_fact WHERE id = ?', [id]);
};
...
const getFactureNById = async (fournisseur) => {
  const [rows] = await db.query('SELECT * FROM facture WHERE fournisseur = ?', [fournisseur]);
  if (rows.length === 0) {
    throw new Error('Facture not found');
  }
  return rows[0];  
...
const postReclamation = async ( title, description, email) => {
  await db.query('INSERT INTO reclamation ( title, description, email) VALUES ( ?, ?, ?)', [ title, description, email]);
};
...
module.exports = { getUserByEmail, createUser,  getAllUsers ,updateUserPassword,archiveFacture,
  postFacture ,getAllFactures , postReclamation,getUserById,getFactureById,updateFacture,
    ...
    deleteUser , deleterec, deletefacture,deletearchive ,getFacturesByUserId
  , postFactureP , getAllFacturesP , acceptFacture , deletefactureP , getFactureNById}; 